...
Paste this into your project's `CLAUDE.md`, `.cursorrules`, or `copilot-instructions.md`. It tells your AI coding agent three things: **call `prior_attempts` before editing an entity** (so it doesn't repeat a change the team already reverted), **log `why` it changed code** the moment it does, and **log the paths it *didn't* take** — rejections and reverts, with the condition that would invalidate them.

:::tip[One block, three new habits]
Your agent now checks its own history before it writes, leaves a durable trail of *why* behind every change, and records abandoned alternatives instead of letting them evaporate. `pip install selvedge` first, then drop this in. Or just run `selvedge prompt --install CLAUDE.md` and Selvedge installs (and keeps it updated) for you.
:::
...
- **After deciding against** an approach, it logs a `reject` (v0.3.11) — so the path never taken is on the record too, and the next session's `prior_attempts` returns it as a stated outcome instead of re-deriving the dead end.
