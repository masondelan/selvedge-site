...
description: Selvedge records the reasoning behind every AI-written change — logged live by the agent over MCP into local SQLite, and answerable later with selvedge blame.
structuredData:
  type: techarticle
...
With Selvedge wired in, the agent's [instruction block](/prompt-block/) tells it to log each change as it makes it:
