...
And it reads back: before editing an entity, your agent calls [`prior_attempts`](/prior-attempts/)
to see what was already tried and reverted — active memory, so it doesn't repeat a mistake the
team already undid. Full case study, with the exact tool call and what the agent did instead:
