...
<CardGrid>
  ...
  <Card title="Stays local, stays simple" icon="seti:lock">
    ...
    next to your code. No accounts, no servers, and your code never leaves the machine.
  </Card>
</CardGrid>

And it reads back. Before editing an entity, your agent calls
[`prior_attempts`](/prior-attempts/) — and gets back what was already tried there, and
how it ended:
...
So the agent doesn't re-add a column the team already pulled — it plans around the
reason instead. No line-attribution tool surfaces a rejected path at all.
[Full case study, with the plan it chose instead →](/prior-attempts/)

More direct answers: [the MCP server that tracks *why* your agent changed code](/track-why/) ·
[long-term memory for AI coding agents](/agent-memory/) ·
[a local decision audit trail](/audit-trail/) ·
[codebase-memory MCP servers, compared](/compare/codebase-memory-mcp-servers/)
...
<CardGrid>
  <LinkCard title="Cursor" description="Paste the MCP config, or run selvedge setup." href="/mcp/cursor/" />
  <LinkCard title="VS Code" description="A .vscode/mcp.json block." href="/mcp/vscode/" />
...
again?" — that's the question Selvedge already has the answer to.

---

## Where Selvedge fits

AI agents call Selvedge as they work. It captures the *why* into a durable, queryable
store — and exports it in the open [Agent Trace](/compare/agent-trace/) format, so the
reasoning travels into whatever observability or audit tooling you already run.
...
features. It sits between them — the layer that records *why*, so git, review, and
observability tools finally have something to point at.
